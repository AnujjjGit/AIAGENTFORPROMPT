...
def evaluate_prompt(spec: PromptSpec, compiled_prompt: str) -> QualityReport:
    context_separated = (not spec.selected_context) or (
        "Context" in compiled_prompt or "<context>" in compiled_prompt
    )
    checks = {
    ...
    if not checks["constraints_explicit"]:
        recommendations.append(
            "Add hard constraints or preferences when they matter to task success."
        )
    if not checks["output_defined"]:
        recommendations.append(
            "Specify the desired output shape when downstream structure matters."
        )
